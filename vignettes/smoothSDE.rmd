...
# To-do list
 ...
 - Possible improvement to make it easier to set fixpar and par0 (and more?): save ncol_fe somewhere in the object. Or save a vector of length ncol(X_fe), where the value of each entry indicates which SDE parameter it corresponds to. For example, c(1, 1, 2, 3) if the first two columns of X_fe are for the first SDE parameter, the third column is for the second parameter, and the fourth column is for the third parameter.
